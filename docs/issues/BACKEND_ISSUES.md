# EO-churn (model + backend) — Issue listesi

Denetim bulgularından ve template planından çıkarıldı. Her blok doğrudan GitHub
issue olarak kopyalanabilir.

**Etiket şeması** (önce bunları oluştur):

| Etiket | Anlamı |
|---|---|
| `P0-demo` | Canlı demoda görünür / demo öncesi kapanmalı |
| `P1-pilot` | Müşteri pilotu öncesi pazarlıksız |
| `P2` | Borç, sırası gelince |
| `area:db` `area:api` `area:ml` `area:ops` `area:notify` `area:docs` | Alan |
| `template` | Template refactor'ünün parçası |

İsterseniz hepsini tek seferde oluşturmak için:
...
gh label create area:ml   --color 5319e7
gh label create area:ops  --color 006b75
gh label create area:notify --color c2e0c6
gh label create area:docs --color bfd4f2
gh label create template  --color e99695
```

---

## B-01 · Telegram bot token'ı hata mesajıyla ekrana ve log'a düşüyor
...
## B-02 · `demo_reset.sh` tek koşu bırakıyor, trend özelliği demoda hiç görünmüyor
...
## B-03 · `daily_students` tarihli hale gelsin (`as_of_date`)
...
## B-04 · `runs` tablosu: koşu kimliği ve boş koşu kaydı
...
## B-05 · `alerts`'e feature snapshot + `model_version` + `threshold`
...
## B-06 · Sonuç (label) ve müdahale kayıt tabloları
...
## B-07 · Kimlik doğrulama fail-closed olsun
...
## B-08 · `POST /run-daily-pipeline`'ı HTTP yüzeyinden kaldır
...
## B-11 · `to_native` tüm çıktı yollarında uygulanmalı
...
## B-12 · Öğrenci id'leri log'dan ve hata gövdelerinden çıkar
...
## B-13 · Yedekleme: `pg_dump` + zamanlama + geri yükleme prosedürü
...
## B-14 · Zamanlayıcı servisi + koşu başarısızlığında bildirim
...
## B-15 · CI'a Postgres servisi — DB testleri gerçekten koşsun
...
## B-16 · Migration sürüm tablosu + `001`'in idempotent olmaması
...
## B-17 · Container hijyeni: dockerignore, non-root, sabitlenmiş base
...
## B-18 · KVKK: silme / anonimleştirme yolu
...
## B-19 · `daily_students` budama / `is_active`
...
## B-20 · Hazırlık (readiness) doğru olsun; kalibratör eksikse fail-closed
...
## B-21 · Sızıntı denetimi: iletişim feature'ları ve `days_to_next_exam`
...
## B-22 · Temiz split'ler: 4. dilim, imputer split sonrası, zamana göre bölme
...
## B-23 · Eşik ile mentor kapasitesini uzlaştır
...
## B-24 · Model sürümleme, rollback ve drift kontrolü
...
## B-26 · ML çekirdeği için test yok
...
## B-27 · Platt kalibrasyonu log-odds üzerinde fit edilmeli
...
## B-28 · Satır bazlı karantina — tek null tüm koşuyu düşürmesin
...
## B-29 · Template: `clients/` iskeleti + `ClientProfile`
...
## B-30 · Template: testler profil fixture'ına bağlanmalı
...
## B-31 · Template: entity soyutlaması
...
## B-32 · Template: `GET /schema` endpoint'i
...
## B-33 · Template: mesaj sözlüğü, türetim kaydı ve kalan sabitler
...
## B-34 · Template: `new_client.py` scaffolder + `docs/ONBOARDING.md`
...
## B-35 · Template: ikinci dikeyi gerçekten yap (OULAD)
...
## B-36 · `docs/DEMO_SETUP.md` — ekip arkadaşı için kurulum sayfası
